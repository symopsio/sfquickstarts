...
id: snowflake-agentic-ai-in-supply-chain
summary: Modern supply chain operations face a critical challenge: efficiently managing raw material inventory across multiple manufacturing facilities.
...
4. Select the **SUPPLY\_CHAIN\_ASSISTANT\_PDF\_STAGE** stage
...
1. In the same Stages view, select the **SEMANTIC\_MODELS\_STAGE** stage
2. Click the **+ Files** button
3. Upload both semantic model files:

   * **scripts/semantic\_models/SUPPLY\_CHAIN\_ASSISTANT\_MODEL.yaml**
   * **scripts/semantic\_models/WEATHER\_FORECAST.yaml**
...
2. Import the **scripts/configure\_search\_services.sql** file from the quickstart repository
3. Click **Run All** to execute the script

This script will:

* Parse PDFs using Cortex PARSE\_DOCUMENT function
...
> Ensure you are using SUPPLY\_CHAIN\_ASSISTANT\_ROLE in Snowsight
...
4. * **Schema** `AGENTS`
5. **Agent object name** **Supply\_Chain\_Agent**
...
> aside negative
> **Note:** The WEB\_SEARCH and WEB\_SCRAPE custom tools require external integration access, which is not available on trial accounts. If you're using a trial account, you can skip adding these two tools and still use the other agent capabilities.

For each of the following custom tools, click **+ Add** next to **Custom Tool**, then configure:

#### 1. CREATE\_HTML\_NEWSLETTER
...
4. Select your **Supply\_Chain\_Agent** from the dropdown
...
> aside negative
> **Note:** Sending emails requires a verified email addresses. The WEB\_SEARCH and WEB\_SCRAPE tools require external integration access, which is not available on trial accounts.
